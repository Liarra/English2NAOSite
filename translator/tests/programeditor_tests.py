...
class ProgramEditorTests(TestCase):
    ...
    def test_update_state_no_input_states_stay_the_same(self):
        ...
        new_states = ProgramEditor.update_state(sample_states, '1.00', [], [], [], [], [], [])
        self.assertEqual(sample_states, new_states)

        new_states = ProgramEditor.update_state(sample_states, '1.01', [], [], [], [], [], [])
        self.assertEqual(sample_states, new_states)

        new_states = ProgramEditor.update_state(sample_states, '1.02', [], [], [], [], [], [])
        self.assertEqual(sample_states, new_states)

        new_states = ProgramEditor.update_state(sample_states, '2.00', [], [], [], [], [], [])
        self.assertEqual(sample_states, new_states)

        new_states = ProgramEditor.update_state(sample_states, '2.01', [], [], [], [], [], [])
        self.assertEqual(sample_states, new_states)

        new_states = ProgramEditor.update_state(sample_states, '3.00', [], [], [], [], [], [])
        self.assertEqual(sample_states, new_states)

    def test_update_state_remove_action(self):
        sample_states = self.getSampleStates()
        new_states = ProgramEditor.update_state(sample_states, '1.00', actions_to_remove=[0])
        new_commands = new_states[0][0].commands
        self.assertTrue(len(new_commands) == 0)

        new_states = ProgramEditor.update_state(sample_states, '1.01', actions_to_remove=[0])
        new_commands = new_states[0][1].commands
        self.assertTrue(len(new_commands) == 0)

        new_states = ProgramEditor.update_state(sample_states, '2.01', actions_to_remove=[0])
        ...
        new_states = ProgramEditor.update_state(sample_states, '3.00', actions_to_remove=[0])
    ...
    def test_update_state_remove_condition(self):
        sample_states = self.getSampleStates()
        new_states = ProgramEditor.update_state(sample_states, '1.00', conditions_to_remove=[0])
        new_state = new_states[0][0]
        self.assertIsInstance(new_state, State)
        self.assertNotIsInstance(new_state, ConditionState)

        new_states = ProgramEditor.update_state(sample_states, '2.01', conditions_to_remove=[0])
    ...
    def test_update_state_change_actions(self):
        ...
        new_states = ProgramEditor.update_state(sample_states, '1.00', change_actions=change_actions)
        ...
        new_states = ProgramEditor.update_state(sample_states, '2.01', change_actions=change_actions)
        new_command = new_states[1][1].commands
        self.assertEquals(1, len(new_command))
        self.assertEquals('[say(Omg Yes Yes Yes)]', str(new_command[0]))

        new_states = ProgramEditor.update_state(sample_states, '1.01', change_actions=change_actions)
    ...
    def test_update_state_change_conditions(self):
        ...
        new_states = ProgramEditor.update_state(sample_states, '1.00', change_conditions=change)
        new_condition = new_states[0][0].condition
        self.assertEquals(1, len(new_condition))
        self.assertEquals('key[b]->', str(new_condition[0]))

        new_states = ProgramEditor.update_state(sample_states, '2.01', change_conditions=change)
    ...
    def test_update_state_add_actions(self):
        ...
        new_states = ProgramEditor.update_state(sample_states, '1.01', actions_to_add=addition)
        new_command = new_states[0][1].commands
    ...
    def test_update_state_add_conditions(self):
        ...
        new_states = ProgramEditor.update_state(sample_states, '3.00', conditions_to_add=addition)
        ...
        new_states = ProgramEditor.update_state(sample_states, '1.00', conditions_to_add=addition)
        new_state = new_states[0][0]
        self.assertIsInstance(new_state, ConditionState)
    ...
    def test_update_state_mixed_change(self):
        ...
        new_states = ProgramEditor.update_state(sample_states, '4.00',
